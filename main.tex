...
\begin{proof}
  ...
  \end{equation}
  Here, $H$ is precisely the Gram matrix of $(\tilde{r}_{n-1}, \tilde{r}_{n})$. By \Cref{lem:projection}(i), $P_{S_{-i,-j}} v_{n-1} = \sum_{k} c^{(n-1)}_{k} v_{k}$ with $c^{(n-1)} = A^{-1} B_{\cdot,n-1}$. Because the error $\tilde{r}_{n-1}$ is orthogonal to the control subspace $S_{-i,-j}$ while the projection $v_{n} - \tilde{r}_{n} \in S_{-i,-j}$, we get:
  \begin{equation*}
    \begin{aligned}
      \inner{\tilde{r}_{n-1}, \tilde{r}_{n}}
      &= \inner{\tilde{r}_{n-1}, v_{n}} \\
      &= D_{n-1,n} - (c^{(n-1)})^{\prime} B_{\cdot,n} \\
      &= D_{n-1,n} - B_{\cdot,n-1}^{\prime} A^{-1} B_{\cdot,n} \\
      &= H_{n-1,n}
    \end{aligned}
  \end{equation*}
